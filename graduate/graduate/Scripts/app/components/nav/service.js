...
define([
    ...
    return angular.module("components.nav.services", ['Common.services'])
        .service('navNetService', ['httpRequestWapper', 'singleHttpFactory', function (httpRequestWapper, singleHttpFactory) {
            return {
                uploadUserHead: singleHttpFactory.post('/api/userService/updateUserHeadImgUrl'),
                changePassword: singleHttpFactory.post('/api/UserService/UpdateUserPwd'),
                logout: singleHttpFactory.get('/api/LoginService/Logout'),
                uploadImage: singleHttpFactory.post('/api/userService/upLoadHeadImg')
                //openCrm: singleHttpFactory.post('/UserService/RedirectToCrm'),
                //setLanguage: singleHttpFactory.post('/UserService/SetLanguage')
            };
        }])
